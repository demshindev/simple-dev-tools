...
export default function TimestampConverter() {
  ...
  useEffect(() => {
    if (mode === 'to-datetime') {
      ...
      if (num < 10000000000) {
        const ms = num * 1000
        if (ms > maxTimestamp || ms < minTimestamp) {
          setDatetime('Invalid date: value out of range')
          return
        }
        date = new Date(ms)
      } else {
        if (num > maxTimestamp || num < minTimestamp) {
          setDatetime('Invalid date: value out of range')
          return
        }
        date = new Date(num)
      ...
      try {
        ...
        setDatetime('Invalid date: conversion error')
      }
    }
  }, [timestamp, mode])
